...
import {
  GenericMeasureStatic,
  getGenericMeasureStaticMethods,
} from "./genericMeasureStatic";
...
type DimensionResult<
  N,
  D extends string,
> = true extends IsSingleStringLiteral<D>
...
interface GenericMeasureFactory<N> {
  ...
  of<U extends Unit>(
    value: N,
    quantity: GenericMeasure<N, U>,
    symbol?: string,
  ): GenericMeasure<N, U>;
}

type GenericMeasureCommon<N> = GenericMeasureFactory<N> &
 ...
 */
export type GenericMeasureType<
  N,
  StaticMethods extends object,
> = GenericMeasureCommon<N> &
 ...
 */
export function createMeasureType<N, S extends object = object>(
  ...
  const common: GenericMeasureCommon<N> = {
    ...getGenericMeasureStaticMethods(),
    isMeasure: (value): value is GenericMeasure<N, any> =>
      value instanceof Measure,
    dimensionless: (value) => new Measure(value, {}),
    dimension: <D extends string>(dim: D, symbol?: string) =>
      new Measure(num.one(), dimension(dim, symbol), symbol) as DimensionResult<
        N,
        D
      >,
    of: <U extends Unit>(
